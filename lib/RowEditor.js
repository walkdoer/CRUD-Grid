...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
    ...
    buttonPad: 3,
    clicksToEdit: 2, //'auto',
    noClicksToEdit: false,
    monitorValid: true,
    focusDelay: 250,
    errorSummary: false,//do not show error msg
    ...
    startEditing: function (rowIndex, doFocus) {
        ...
        if (this.fireEvent('beforeedit', this, rowIndex) !== false) {
            ...
            //var typeNotRender = ['numberfield', 'actioncolumn'];
            for (var i = 0, len = cm.getColumnCount(); i < len; i++) {
                val = this.preEditValue(record, cm.getDataIndex(i));
                f = fields[i];
                /*
                var column = cm.getColumnById(cm.getColumnId(i));
                if (column.renderer && typeNotRender.indexOf(column.xtype) < 0
                    && (!column.editor || typeNotRender.indexOf(column.editor.getXType()) < 0)) {
